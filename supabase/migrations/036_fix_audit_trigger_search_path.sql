-- Fix: audit_trigger() uses digest() from pgcrypto, which Supabase installs
-- into the 'extensions' schema. The previous search_path excluded it, causing
-- "function digest(text, unknown) does not exist" on every audited INSERT/UPDATE/DELETE.
